...
pos= r"HR's requirement: " + position1[0].upper()
try:
    query2=f"""Select skill from skills where LOWER(position)='{position1[0].lower()}'"""
    cur.execute(query2)
    skills = cur.fetchall()
    mydb.commit()
    ski=list(skills[0])
    skill=r"Main keywords:\n " + ski[0].upper()
except:
    ski = ['']
    skill=r"Main keywords:\n None"
        ...
        #If category matches the requirement    
            if dict_category[pred[0]].lower()==position1[0].lower():
                ...
                #Storing it in database (MYSQL)    
                    ...
                    value = (fullName,email, content,score,location,prediction_category.upper())
